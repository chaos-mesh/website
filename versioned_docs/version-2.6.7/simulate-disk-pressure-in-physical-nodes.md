...
To create experiments using the service mode, you need to run Chaosd in the service mode and then send a `POST` HTTP request to the `/api/attack/disk` path of the Chaosd service:
...
| :-- | :-- | :-- | :-- | :-- | :-- |
| `action` | — | `action` | Actions of the experiment | string | Set to `"read-payload"` |
| `path` | p | `path` | Specifies the file path to read the data. If this parameter is not specified, or the parameter value is set to an empty string, Chaosd reads from the virtual disk files mounted in the "/" directory. Depending on the permissions to read the files, you might be required to run this program using certain permissions. | string | Default: `""` |
| `process-num` | n | `payload-process-num` | Specifies the number of concurrently running [dd](https://man7.org/linux/man-pages/man1/dd.1.html) programs to be used. | uint8 | Default: `1`; range: `1` to `255` |
| `size` | s | `size` | Specifies the volume of data to be read. It is the total size of data that dd reads. | string | Default: `""`; **required**; legal form: the combination of an integer and a unit. For example, 1M, 512kB. Supported units are c=1, w=2, b=512, kB=1000, K=1024, MB=1000\*1000, M=1024\*1024, GB=1000\*1000\*1000, G=1024\*1024\*1024\*1024 BYTE and so on. |
...
Run the following command to see the commands for simulating disk read load:
...
The example for simulating disk read load is as follows:
...
Send a `POST` HTTP request to the `/api/attack/disk` path of the Chaosd service with the following `fault-configuration`:
...
| Configuration item | Abbreviation | Service mode field | Description | Type | Value |
| :-- | :-- | :-- | :-- | :-- | :-- |
| `action` | — | `action` | Actions of the experiment | string | Set to `"write-payload"` |
| `path` | p | `path` | Specifies the file path to write the data. If this parameter is not specified, or the parameter value is set to an empty string, a temporary file will be created in the program execution directory. Depending on the permissions to write the files, you might be required to run this program using certain permissions. | string | Default: `""` |
| `process-num` | n | `payload-process-num` | Specifies the number of concurrently running [dd](https://man7.org/linux/man-pages/man1/dd.1.html) programs to be used. | uint8 | Default: `1`; range: `1` to `255` |
| `size` | s | `size` | Specifies the volume of data to be written. It is the total size of data that dd writes. | string | Default: `""`; **required**; legal form: the combination of an integer and a unit. For example, 1M, 512kB. Supported units are c=1, w=2, b=512, kB=1000, K=1024, MB=1000\*1000, M=1024\*1024, GB=1000\*1000\*1000, G=1024\*1024\*1024\*1024 BYTE and so on. |

### Simulate disk write load using the command-line mode

Run the following command to see the command for simulating disk write load:
...
The example for simulating disk write load is as follows:
...
| Configuration item | Abbreviation | Service mode field | Description | Type | Value |
| :-- | :-- | :-- | :-- | :-- | :-- |
| `action` | — | `action` | Actions of the experiment | string | Set to `"fill"` |
| `destroy` | d | `destroy` | If this parameter is set to `true`, the fill file is immediately deleted after being filled. | bool | Default: `false` |
| `fallocate` | f | `fill-by-fallocate` | If this parameter is set to `true`, Linux is used to call fallocate to quickly apply for disk space and size must be greater than `0`. If this parameter is set to `false`, Linux is used to call dd to fill disks at a relatively slow pace. | bool | Default: `true` |
| `path` | p | `path` | Specifies the file path to write the data. If this parameter is not specified, or the parameter value is set to an empty string, a temporary file will be created in the program execution directory. Depending on the permissions to write the files, you might be required to run this program using certain permissions. | string | Default: `""` |
| `percent` | c | `percent` | Specifies the percentage of disk size to be filled. | string | Default: `""`; positive integer of the uint type is acceptable; you must set one of `size` or `percent` (both items **cannot** be `""` at the same time). |
| `size` | s | `size` | Specifies the volume of data to be written. | string | Default: `""`; legal form: the combination of an integer and a unit. For example, 1M, 512kB. Supported units are c=1, w=2, b=512, kB=1000, K=1024, MB=1000\*1000, M=1024\*1024, GB=1000\*1000\*1000, G=1024\*1024\*1024\*1024 BYTE and so on. You must set one of `size` or `percent` (both items **cannot** be `""` at the same time). |

### Simulate disk fill using the command-line mode

Run the following command to see the command for simulating disk fill:
...
The example for simulating disk fill is as follows:
