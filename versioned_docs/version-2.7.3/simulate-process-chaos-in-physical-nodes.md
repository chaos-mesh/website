...
| :-- | :-- | :-- | :-- | :-- | :-- |
| `process` | p | process | The name or the identifier of the process into which faults are injected | string | Default value: `""` |
| `recover-cmd` | r | recover-cmd | The command to be run when recovering experiment | string | Default value: `""` |
| `signal` | s | signal | The provided value of the process signal | int | Default value: `9`. Currently, only `SIGKILL`, `SIGTERM`, and `SIGSTOP` are supported. |

### Simulate killing a process using the command-line mode
...
### Simulate killing a process using the service mode
...
### Simulate stopping a process using the service mode
