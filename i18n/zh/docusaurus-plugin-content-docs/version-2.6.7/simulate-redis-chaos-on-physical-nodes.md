...
目前 Chaosd 支持模拟缓存过期、缓存穿透、缓存限流、哨兵重启和哨兵不可用等故障场景。
...
| `expiration` | — | `expiration` | 指定的键值对将会在到达 `expiration` 之后过期 | string 类型 | 请确保输入的字符串为 `time.Duration` 支持的格式，默认为 `0` |
| `key` | k | `key` | 要设置过期时间的键 | string 类型 | 默认为 `""`。当该值为默认时，将对所有键设置过期时间 |
| `option` | — | `option` | 对 `expiration` 的额外操作，用于设置键的过期条件。**只有 Redis 7.0.0 之后的版本支持该参数** | string 类型 | 默认为 `""`。只支持 NX，XX，GT，LT |
| `password` | p | `password` | 登录 Redis 服务器的密码 | string 类型 | 默认为 `""` |

### 使用命令行模式模拟缓存过期场景

运行以下命令可查看模拟缓存过期支持的配置：
...
向 Chaosd 服务的路径 `/api/attack/redis` 发送 `POST` HTTP 请求，并配置如下 `fault-configuration`：
...
| `addr` | a | `addr` | 需要注入故障的 Redis 服务器的地址以及端口号，如 `127.0.0.1:6379` | string 类型 | 默认为 `""` |
| `password` | p | `password` | 登录 Redis 服务器的密码 | string 类型 | 默认为 `""` |
| `percent` | — | `percent` | 指定 `maxmemory` 为原值的百分比 | string 类型 | 默认为 `""` |
| `size` | s | `cacheSize` | 指定 `maxmemory` 的大小 | string 类型 | 默认为 `0`，`0` 表示不限制内存大小 |

### 使用命令行模式模拟缓存限流场景

运行以下命令可查看模拟缓存限流支持的配置：
...
| `addr` | a | `addr` | 需要注入故障的 Redis 服务器的地址以及端口号，如 `127.0.0.1:6379` | string 类型 | 默认为 `""` |
| `password` | p | `password` | 登录 Redis 服务器的密码 | string 类型 | 默认为 `""` |
| `request-num` | — | `requestNum` | 指定向 Redis 服务器发送的无效请求数 | int 类型 | 默认为 `0` |

### 使用命令行模式模拟缓存穿透场景

运行以下命令可查看模拟缓存穿透支持的配置：
...
| `flush-config` | — | `flushConfig` | 指定在哨兵重启前，是否将内存中的配置更新到配置文件中 | bool 类型 | 默认为 `true` |
| `password` | p | `password` | 登录 Redis Sentinel 的密码 | string 类型 | 默认为 `""` |
| `redis-path` | — | `redisPath` | 指定 `redis-server` 命令的路径 | string 类型 | 默认为 `""` |

### 使用命令行模式模拟哨兵重启场景

运行以下命令可查看模拟哨兵重启支持的配置：
