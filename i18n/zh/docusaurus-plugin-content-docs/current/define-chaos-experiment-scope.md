...
`PhysicalMachine` 是一种代表物理机的 CRD (CustomResourceDefinition)。要创建 `PhysicalMachine`，请使用 `chaosctl pm create` 或 `chaosctl pm init` 命令。更多详情，请参见 [Chaosctl](chaosctl-tool.md)。
