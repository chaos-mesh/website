...
以服务模式运行 chaosd，通过向 chaosd 服务的路径 `/api/attack/process` 发送 HTTP POST 请求，从而创建实验。在运行实验时，请注意保存实验的 uid 信息。

向 chaosd 服务的路径 `/api/attack/{uid}` 发送 HTTP DELETE 请求来结束 uid 对应的实验。
