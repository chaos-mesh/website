...
1. 将实验配置写入到文件 `azurechaos-disk-detach.yaml` 中，内容如下所示：
   ...
   如需查看更多关于分离 Azure 数据磁盘的信息，可参考[分离数据磁盘](https://docs.microsoft.com/zh-cn/azure/devtest-labs/devtest-lab-attach-detach-data-disk#detach-a-data-disk)。
...
| vmName | string | 指定 VM 的名称 | 无 | 是 | `your-vm-name` |
| diskName | string | 当 action 为 `disk-detach` 时必填，指定数据磁盘的名称 | 无 | 否 | `DATADISK_0` |
| lun | string | 当 action 为 `disk-detach` 时必填，指定硬盘的 LUN (Logical Unit Number) | 无 | 否 | `0` |
