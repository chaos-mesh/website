...
`RemoteCluster` 尚处于早期阶段。它的配置和功能（例如配置迁移、版本管理和认证）将不断完善。如果您遇到任何问题，请在 [chaos-mesh/chaos-mesh](https://github.com/chaos-mesh/chaos-mesh) 中提交一个 issue 进行反馈。
...
它将在指定命名空间中使用 `.spec.kubeConfig` 字段提供的 `KUBECONFIG` 安装 `chaos-mesh` Helm chart。

### 字段说明

| 参数 | 类型 | 说明 | 默认值 | 是否必填 | 示例 |
| --- | --- | --- | --- | --- | --- |
| namespace | string | 表示在远程集群中安装 Chaos Mesh 组件的命名空间 | 无 | 是 | chaos-mesh |
...
Chaos Mesh 将使用注册到名为 `cluster-xxx` 的 `RemoteCluster` 的 kubeconfig 将故障注入到远程集群。相应的 `StressChaos` 将在远程集群中自动创建，并将状态同步回当前集群，以便您可以在单个 Kubernetes 集群中管理多个不同集群的故障注入。
