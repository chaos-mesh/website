...
Chaos Mesh v2.1.0 introduces the following significant changes, which expand the user scenarios of Chaos Mesh and improves overall usability:

- Support creating and running Chaos experiments in a physical machine, an important step for Chaos Mesh to become a platform.
- Optimize the workflow to display the workflow-related events and configurations and to create HTTP tasks in the workflow, which greatly adds the user scenarios of Workflow.
...
- Integrates GCP certification in Chaos Dashboard.
...
In this section, you are going to deploy Kubernetes and Chaos Mesh as follows:
...
2. Deploy Kubernetes and Chaos Mesh
...
`install.sh` is a one-click deployment script provided by Chaos Mesh. The script automatically checks your environment, installs [kind](https://kind.sigs.k8s.io), starts a local Kubernetes cluster, and then installs Chaos Mesh.
...
4. Run the following command to access kind-control-planeand send HTTP requests:
