...
3. If the APIs & services page was not loaded automatically, open the left side menu of the console and select "APIs & services" manually.
4. Click "Credentials" on the left.
5. Click "Create Credentials", then select "OAuth client ID".
6. Select "Web Application" as the application type, and enter additional information and the redirect URL of Chaos Dashboard, which is `ROOT_URL/api/auth/gcp/callback`. Here, `ROOT_URL` is the root URL of Chaos Dashboard, such as `http://localhost:2333`. This URL can be set through the configuration item `dashboard.rootUrl` by `helm`.
7. Click "Create".
...
Update: Since `v2.7.0`, you can provide a **Secret** to store the Client ID and Client Secret. **We recommend using this method**.

This change is to avoid exposing the Client ID and Client Secret to the public. In previous versions, the Client ID and Client Secret were specified directly in the values, which is not safe in general.
...
If Chaos Mesh has already been installed, you can update the configuration items through `helm upgrade`. If not, you can install Chaos Mesh through `helm install`.
...
By setting this configuration, the user `example@gmail.com` can see and create any chaos experiments.
