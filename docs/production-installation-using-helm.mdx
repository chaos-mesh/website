...
Before installing Chaos Mesh, make sure that:

- Your Kubernetes cluster runs version `>= v1.16`. Chaos Mesh is continuously tested in CI against Kubernetes up to `v1.35.x`. For the versions covered by each release, see [Supported Releases](/supported-releases).
- You have installed [Helm](https://helm.sh/docs/intro/install/) in your environment.

To check whether Helm is installed, execute the following command:
...
In this document, Helm v3 is used to operate Chaos Mesh. If Helm v2 is used in your environment, refer to [Migrating Helm v2 to v3](https://helm.sh/docs/topics/v2_v3_migration/) or modify the Helm version to the v2 format.
...
The above command will output the latest release of the chart. If you want to install a historical version, execute the following command to view all released versions:
...
To modify the configuration, set different values according to your needs. For example, execute the following command to upgrade and uninstall `chaos-dashboard`:
...
<PickVersion>{`curl -sSL https://mirrors.chaos-mesh.org/latest/crd.yaml | kubectl apply --server-side -f -`}</PickVersion>
