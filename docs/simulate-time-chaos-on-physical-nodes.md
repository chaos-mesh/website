...
| :-- | :-- | :-- | :-- | :-- | :-- |
| `time-offset` | `-t` | time-offset | Specifies the length of time offset. | string | None; required. For example, `-5m`. |
| `clock-ids-slice` | `-c` | clock-ids-slice | Specifies the ID of the clock that will be offset. Multiple clock IDs should be separated by a comma. See the [clock_gettime documentation](https://man7.org/linux/man-pages/man2/clock_gettime.2.html) for details. | string | Default value: `"CLOCK_REALTIME"`. |
| `pid` | `-p` | pid | The identifier of the process. | int | None; required. |
